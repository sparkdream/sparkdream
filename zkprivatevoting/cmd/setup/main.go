// Command setup generates the proving and verifying keys for the vote circuit.
//
// Usage:
//
...
//
// This will create:
//   - keys/proving_key.bin   - Used by voters to generate proofs (~50-100 MB)
...
func main() {
	...
	fmt.Println("Private Voting ZK Circuit - Trusted Setup")
	fmt.Println()
	...
	info("  Tree depth: %d (max %d voters)", circuit.TreeDepth, 1<<circuit.TreeDepth)
	fmt.Println()
	...
	fmt.Printf("  %s/\n", *outputDir)
	fmt.Printf("     proving_key.bin     (%s) - Distribute to voters\n", formatBytes(pkBytes))
	fmt.Printf("     verifying_key.bin   (%s) - Embed in chain\n", formatBytes(vkBytes))
	...
	fmt.Println("  1. Embed verifying_key.bin in your chain's genesis or params")
	fmt.Println("  2. Distribute proving_key.bin to voters via your client app")
	...
	fmt.Println("    running an MPC ceremony with multiple participants.")
	fmt.Println()
}

// testProofGeneration creates and verifies a test proof
func testProofGeneration(
	...
	votingPower := uint64(1000)
	proposalID := uint64(1)
	voteOption := uint64(0) // Yes

	// Create Merkle tree with this voter
	leaf := crypto.ComputeLeaf(keys.PublicKey, votingPower)
	...
	// Compute nullifier
	nullifier := crypto.ComputeNullifier(keys.SecretKey, proposalID)
