...
func (k msgServer) RevokeBridge(ctx context.Context, msg *types.MsgRevokeBridge) (*types.MsgRevokeBridgeResponse, error) {
	...
	if bridge.Status != types.BridgeStatus_BRIDGE_STATUS_ACTIVE &&
		...
		return nil, errorsmod.Wrapf(types.ErrBridgeNotActive, "bridge status is %s, cannot revoke", bridge.Status)
	}
