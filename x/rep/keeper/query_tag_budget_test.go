...
func createNTagBudget(k keeper.Keeper, ctx context.Context, n int) []types.TagBudget {
	...
	for i := range items {
		...
		items[i].Active = true
		_ = k.TagBudget.Set(ctx, iu, items[i])
		_ = k.TagBudgetSeq.Set(ctx, iu)
	}
	return items
}
...
func TestTagBudgetQueryPaginated(t *testing.T) {
	...
	req := func(next []byte, offset, limit uint64, total bool) *types.QueryAllTagBudgetRequest {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(msgs); i += step {
			resp, err := qs.ListTagBudget(f.ctx, req(nil, uint64(i), uint64(step), false))
			require.NoError(t, err)
	...
	t.Run("Total", func(t *testing.T) {
		resp, err := qs.ListTagBudget(f.ctx, req(nil, 0, 0, true))
		require.NoError(t, err)
