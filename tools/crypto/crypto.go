...
// ComputeLeaf computes a Merkle tree leaf.
// leaf = hash(publicKey, metadata)
//
// For the trust tree: metadata = trustLevel
// For legacy voter trees: metadata = votingPower
func ComputeLeaf(publicKey []byte, metadata uint64) []byte {
	metadataBytes := Uint64ToBytes(metadata)
	return HashToField(publicKey, metadataBytes)
}

// ComputeRateLimitNullifier computes the per-identity rate-limit nullifier.
// rateLimitNullifier = hash(secretKey, domainTag, epoch)
//
// The domain tag (MaxUint64) separates this from action nullifiers.
// All operations by the same member in the same epoch produce the same
// rate-limit nullifier, enabling per-identity rate limiting without
// breaking anonymity.
//
// The domain tag must match circuit.RateLimitDomainTag.
func ComputeRateLimitNullifier(secretKey []byte, epoch uint64) []byte {
	domainTag := Uint64ToBytes(^uint64(0)) // math.MaxUint64
	epochBytes := Uint64ToBytes(epoch)
	return HashToField(secretKey, domainTag, epochBytes)
}
