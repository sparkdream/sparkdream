...
HAS_CREATE=$(echo "$PERMS" | jq -r '.policy_permissions.allowed_messages[] | select(. == "/sparkdream.rep.v1.MsgCreateTagBudget")' 2>/dev/null)
HAS_TOGGLE=$(echo "$PERMS" | jq -r '.policy_permissions.allowed_messages[] | select(. == "/sparkdream.rep.v1.MsgToggleTagBudget")' 2>/dev/null)
HAS_WITHDRAW=$(echo "$PERMS" | jq -r '.policy_permissions.allowed_messages[] | select(. == "/sparkdream.rep.v1.MsgWithdrawTagBudget")' 2>/dev/null)
...
if submit_and_exec_commons_proposal "$PROPOSAL_FILE"; then
    ...
    if [ -z "$TAG_BUDGET_ID" ] || [ "$TAG_BUDGET_ID" == "null" ]; then
        # Fallback: query the tag budget by tag name (sequence is 1-indexed in x/rep)
        TAG_BUDGET_ID=$($BINARY query rep list-tag-budget --output json 2>&1 | jq -r ".tag_budget[] | select(.tag == \"$TAG_NAME\") | .id" | head -1)
    fi
...
if submit_and_exec_commons_proposal "$PROPOSAL_FILE"; then
    ...
    if [ -z "$TAG_BUDGET_ID_2" ] || [ "$TAG_BUDGET_ID_2" == "null" ]; then
        # Fallback: query by tag name (sequence is 1-indexed in x/rep)
        TAG_BUDGET_ID_2=$($BINARY query rep list-tag-budget --output json 2>&1 | jq -r ".tag_budget[] | select(.tag == \"$TAG_NAME2\") | .id" | head -1)
    fi
...
RESERVED_TAG_INFO=$($BINARY query rep get-reserved-tag "$RESERVE_TAG" --output json 2>&1)
