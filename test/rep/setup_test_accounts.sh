...
bond_sentinel_if_needed() {
    ...
    STATUS_JSON=$($BINARY query rep bonded-role content-sentinel $($BINARY keys show $ACCOUNT -a --keyring-backend test) --output json 2>&1)
    ...
    TX_RES=$($BINARY tx rep bond-role content-sentinel $AMOUNT \
        --from $ACCOUNT \
        --chain-id $CHAIN_ID \
        --keyring-backend test \
        --fees 5000${BOND_DENOM} \
