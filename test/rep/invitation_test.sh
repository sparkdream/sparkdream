...
if [ "$ALICE_SPARK" -lt "$REQUIRED_SPARK" ]; then
    echo "[FAIL] FAILURE: Alice doesn't have enough SPARK to fund invitees"
    exit 1
fi
...
if [ "$INV1_BALANCE" -gt 0 ] && [ "$INV2_BALANCE" -gt 0 ] && [ "$INV3_BALANCE" -gt 0 ] && [ "$INV4_BALANCE" -gt 0 ]; then
    ...
    echo "[WARN]  Warning: Some invitees may not be funded properly:"
...
if [ -n "$EXISTING_INVITATION" ]; then
    echo "[INFO]  Invitation already exists for Invitee1 (ID: $EXISTING_INVITATION)"
    INVITATION_ID1="$EXISTING_INVITATION"

    if [ -n "$EXISTING_MEMBER" ]; then
        echo "[INFO]  Invitee1 is already a member"
    fi
    ...
    if echo "$ALICE_MEMBER" | grep -q "member"; then
        ...
        if [ "$ALICE_DREAM" -lt 200000000 ]; then
            echo "[WARN]  Warning: Alice has less than 200 DREAM (needs 200000000 micro-DREAM, has $ALICE_DREAM)"
        fi

        if [ "$ALICE_CREDITS" = "0" ] || [ "$ALICE_CREDITS" = "null" ]; then
            echo "[WARN]  Warning: Alice has no invitation credits"
        fi
    else
        echo "[WARN]  Alice is not a member yet (genesis members should be auto-created)"
    fi
    ...
    if [ "$TX_CODE" != "0" ]; then
        echo "[FAIL] Transaction failed with code $TX_CODE"
    ...
    if [ -z "$INVITATION_ID1" ] || [ "$INVITATION_ID1" == "null" ]; then
        echo "[FAIL] FAILURE: Could not extract invitation ID from transaction"
    ...
    echo "[ OK ] Invitation created with ID: $INVITATION_ID1"
fi
...
if [ -z "$EXISTING_MEMBER" ]; then
    ...
    if [ "$STATUS" = "null" ] || [ -z "$STATUS" ]; then
        echo "[ OK ] Invitation status is PENDING (proto3 omits zero-value fields)"
    elif [ "$STATUS" = "INVITATION_STATUS_PENDING" ]; then
        echo "[ OK ] Invitation status is PENDING"
    else
        echo "[FAIL] FAILURE: Invitation should be PENDING, got $STATUS"
        exit 1
    fi
else
    # Member already exists, expect ACCEPTED status
    if [ "$STATUS" = "INVITATION_STATUS_ACCEPTED" ]; then
        echo "[ OK ] Invitation status is ACCEPTED (member already joined)"
    else
        echo "[WARN]  Unexpected status for existing member: $STATUS"
    fi
fi
...
if [ -z "$EXISTING_MEMBER" ]; then
    ...
    if [ "$NEW_STATUS" != "INVITATION_STATUS_ACCEPTED" ]; then
        echo "[FAIL] FAILURE: Invitation should be ACCEPTED, got $NEW_STATUS"
        exit 1
    fi

    echo "[ OK ] Invitation accepted at block: $ACCEPTED_AT"
else
    echo "[INFO]  Invitee1 is already a member, skipping acceptance"
fi
...
if [ "$INVITED_BY" != "$ALICE_ADDR" ]; then
    echo "[FAIL] FAILURE: Invitation chain incorrect"
    exit 1
fi

echo "[ OK ] Member correctly linked to inviter"
...
if echo "$TRANSFER_RES" | jq empty 2>/dev/null; then
    ...
    if [ "$TRANSFER_CODE" != "0" ]; then
        echo "[FAIL] DREAM transfer failed with code $TRANSFER_CODE:"
        echo "   Error: $TRANSFER_RAW_LOG"
    else
        echo "[ OK ] DREAM transfer transaction: $TRANSFER_HASH (code: $TRANSFER_CODE)"
    fi
else
    echo "[WARN]  DREAM transfer error (non-JSON response):"
    echo "$TRANSFER_RES"
fi
...
if [ "$INVITEE1_DREAM" = "0" ]; then
    echo "[WARN]  Warning: DREAM transfer may have failed (invitee1 still has 0 DREAM)"
...
if [ -n "$INVITATION_ID2" ]; then
    echo "[INFO]  Invitation already exists for Invitee2 (ID: $INVITATION_ID2)"
else
    ...
    echo "[ OK ] Invitation created for Invitee2 (ID: $INVITATION_ID2)"
fi
...
if [ "$INVITEE2_EXISTS" = "yes" ]; then
    echo "[INFO]  Invitee2 is already a member"
    ...
    if echo "$ACCEPT_RES" | jq empty 2>/dev/null; then
        ...
        if [ "$TX_CODE" = "0" ]; then
            echo "[ OK ] Invitee2 accepted invitation"
            # Re-check if member was created after acceptance
            sleep 2
            INVITEE2_EXISTS=$($BINARY query rep get-member $INVITEE2_ADDR --output json 2>&1 | grep -q "member" && echo "yes" || echo "no")
        else
            echo "[FAIL] Accept invitation failed with code $TX_CODE"
            echo "$ACCEPT_RES" | jq -r '.raw_log' 2>/dev/null
        fi
    else
        # Response is not JSON (probably an error message)
        echo "[FAIL] Accept invitation failed (non-JSON response):"
        echo "$ACCEPT_RES"
    fi
else
    echo "[INFO]  Invitation status: $INV2_STATUS"
fi
...
if [ "$INVITEE2_EXISTS" = "yes" ]; then
    ...
    if echo "$INVITEE2_MEMBER" | grep -q "member"; then
        ...
        if [ "$CHAIN2_LENGTH" -ne 1 ]; then
            echo "[FAIL] FAILURE: Chain should have 1 member (Alice)"
            echo "Got: $INVITEE2_CHAIN"
            exit 1
        fi

        echo "[ OK ] Invitation chain correctly tracks Alice as inviter"
    else
        echo "[WARN]  Member record not found for Invitee2 (skipping chain validation)"
    fi
else
    echo "[WARN]  Invitee2 not a member yet (skipping chain validation)"
fi
...
if echo "$INVITEE2_MEMBER" | grep -q "member"; then
    ...
    echo "[ OK ] Invitee2 now has $INVITEE2_DREAM micro-DREAM ($(echo "scale=2; $INVITEE2_DREAM / 1000000" | bc) DREAM)"
else
    echo "[WARN]  Skipping DREAM transfer (Invitee2 not a member yet)"
fi
...
if [ -n "$INVITATION_ID3" ]; then
    echo "[INFO]  Invitation already exists for Invitee3 (ID: $INVITATION_ID3)"
else
    ...
    echo "[ OK ] Invitation created for Invitee3 (ID: $INVITATION_ID3)"
fi
...
if [ "$INVITEE3_EXISTS" = "no" ] && [ -n "$INVITATION_ID3" ]; then
    ...
    if echo "$ACCEPT3_RES" | jq empty 2>/dev/null; then
        ...
        if [ "$TX_CODE" = "0" ]; then
            echo "[ OK ] Invitee3 accepted invitation"
            # Re-check if member was created after acceptance
            sleep 2
            INVITEE3_EXISTS=$($BINARY query rep get-member $INVITEE3_ADDR --output json 2>&1 | grep -q "member" && echo "yes" || echo "no")
        else
    ...
    echo "[INFO]  Invitee3 is already a member or no invitation found"
fi
...
if [ "$INVITEE3_EXISTS" = "yes" ] && [ -n "$INVITATION_ID3" ]; then
    ...
    if echo "$INVITEE3_MEMBER" | grep -q "member"; then
        ...
        echo "[WARN]  Member record not found for Invitee3 (skipping chain validation)"
    fi
else
    echo "[WARN]  Invitee3 not ready for chain validation (not a member or no invitation)"
fi
...
if [ -n "$INVITATION_ID4" ]; then
    echo "[INFO]  Invitation already exists for Invitee4 (ID: $INVITATION_ID4)"
else
    ...
    if [ -n "$INVITATION_ID4" ]; then
        echo "[ OK ] Invitation created for Invitee4 (ID: $INVITATION_ID4)"
    else
        echo "[ OK ] Test correctly handled exhausted credits (invitee4 not created - expected)"
    fi
fi
...
if [ -z "$INVITATION_ID4" ]; then
    echo "[ OK ] Invitee4 skipped as expected (Alice exhausted 3 invitation credits)"
    ...
    if echo "$ACCEPT4_RES" | jq empty 2>/dev/null; then
        ...
        if [ "$TX_CODE" = "0" ]; then
            echo "[ OK ] Invitee4 accepted invitation"
            # Re-check if member was created after acceptance
            sleep 2
            INVITEE4_EXISTS=$($BINARY query rep get-member $INVITEE4_ADDR --output json 2>&1 | grep -q "member" && echo "yes" || echo "no")
        else
...
if [ "$INVITEE4_EXISTS" = "yes" ]; then
    ...
    if echo "$INVITEE4_MEMBER" | grep -q "member"; then
        ...
        if [ "$CHAIN4_LENGTH" -ne 1 ]; then
            echo "[FAIL] FAILURE: Chain should have 1 member (Alice only)"
            echo "Got: $INVITEE4_CHAIN"
            exit 1
        fi

        echo "[ OK ] Separate invitation branch correctly tracked"
    else
        echo "[WARN]  Invitee4 member record not found (skipping chain validation)"
    fi
else
    echo "[ OK ] Invitee4 correctly not a member (invitation not created due to exhausted credits)"
fi
...
if [ "$FOUND_COUNT" -ge 3 ] && [ -n "$INVITEE1_FOUND" ] && [ -n "$INVITEE2_FOUND" ] && [ -n "$INVITEE3_FOUND" ]; then
    echo "[ OK ] Alice's invitations found ($FOUND_COUNT/4 test invitees: invitee1, invitee2, invitee3)"
    if [ -z "$INVITEE4_FOUND" ]; then
        echo "[ OK ] Invitee4 correctly not found (Alice exhausted 3 credits as expected)"
    else
        echo "[ OK ] Plus invitee4 also found (Alice had extra credits)"
    fi
else
    echo "[FAIL] FAILURE: Not enough of Alice's invitations found"
...
if [ "$INVITEE1_INV_COUNT" -eq 0 ]; then
    echo "[ OK ] Invitee1 correctly has 0 invitations (new members have no credits)"
else
    echo "[WARN]  Unexpected: Invitee1 has invitations despite being a new member"
fi
...
if [ "$INVITEE2_INV_COUNT" -eq 0 ]; then
    echo "[ OK ] Invitee2 correctly has 0 invitations (new members have no credits)"
else
    echo "[WARN]  Unexpected: Invitee2 has invitations despite being a new member"
fi
...
if [ "$ACCOUNTABILITY_END" == "0" ] || [ "$ACCOUNTABILITY_END" == "null" ]; then
    echo "[FAIL] FAILURE: Accountability period should be set"
    exit 1
fi

echo "[ OK ] Accountability period properly configured"
...
if [ "$INV_EXIST_COUNT" -ge 3 ] && [ -n "$INV1_EXISTS" ] && [ -n "$INV2_EXISTS" ] && [ -n "$INV3_EXISTS" ]; then
    echo "[ OK ] Test invitations found ($INV_EXIST_COUNT/4: invitee1, invitee2, invitee3)"
    ...
    echo "[FAIL] FAILURE: Not enough test invitations found"
...
if echo "$INV1_TAGS" | grep -q "rust" && echo "$INV1_TAGS" | grep -q "golang"; then
    echo "[ OK ] Vouched tags correctly stored"
else
    echo "[WARN]  Vouched tags format: $INV1_TAGS"
fi
...
if [ "$INVITEE1_STATUS" == "MEMBER_STATUS_ACTIVE" ]; then
    echo "[ OK ] Invitee1 is ACTIVE (normal - would be ZEROED if failed accountability)"
else
    echo "[WARN]  Invitee1 status: $INVITEE1_STATUS"
fi
...
echo "[ OK ] Current chain (3 ancestors) is well within limit"
...
if [ "$INV1_REFERRAL_EARNED" == "0" ] || [ "$INV1_REFERRAL_EARNED" == "null" ]; then
    echo "[ OK ] No referral earnings yet (expected - invitee hasn't completed work)"
else
    echo "[ OK ] Referral earnings tracked: $INV1_REFERRAL_EARNED"
fi
...
echo ""
echo "[ OK ] Part 1: Successful invitation flow         ID $INVITATION_ID1"
echo "[ OK ] Part 2: Invitation chain (3 levels)        $CHAIN3_LENGTH/5 max"
echo "[ OK ] Part 3: Separate invitation branch        Chain length: $CHAIN4_LENGTH"
echo "[ OK ] Part 4: Query invitations by inviter       $ALICE_INV_COUNT invitations"
echo "[ OK ] Part 5: Accountability period             Ends at: $ACCOUNTABILITY_END"
echo "[ OK ] Part 6: List all invitations              $TOTAL_INVITATIONS total"
echo "[ OK ] Part 7: Vouched tags verification       Tags: $INV1_TAGS"
echo "[ OK ] Part 8: Failed invitation concept          Status: $INVITEE1_STATUS"
echo "[ OK ] Part 9: Chain depth limit               Max: 5 ancestors"
echo "[ OK ] Part 10: Referral reward concept          Rate: $REFERRAL_RATE"
echo ""
