...
PARAMS_JSON=$($BINARY query split params --output json)
...
# Verify Update
UPDATED_FEE=$($BINARY query split params --output json | jq -r '.params.commons_council_fee')
...
# Verify Reset
FINAL_FEE=$($BINARY query split params --output json | jq -r '.params.commons_council_fee')
