...
$BINARY tx split emergency-cancel-proposal $BAD_PROP_ID --from bob -y --chain-id $CHAIN_ID --keyring-backend test
...
# Verify Final State
FINAL_COUNCIL=$($BINARY query split params --output json | jq -r '.params.commons_council_address')
