...
# Master gate for the entire test-execution loop. Two later invocations
# (dkg_error_test, cross_module_test) sit at column 0 with no per-test gate;
# this master flag lets `--no-tests` skip them too, preventing state drift
# in the `--restore-setup --no-tests` workflow.
RUN_TESTS=true
SAVE_SETUP=false
RESTORE_SETUP=false
...
if [ "$RESTORE_SETUP" = true ]; then
    ...
    # loop, which would drift state via the un-gated tests further down
    # (dkg_error_test, cross_module_test).
    if [ "$RUN_TESTS" != true ]; then
        ...
        echo "    bash $SCRIPT_DIR/registration_test.sh"
...
# RUN_TESTS guard so `--no-tests` skips them too — the early exit in the
# restore branch handles `--restore-setup --no-tests`; this defends against
# `--no-tests` without `--restore-setup`.
if [ "$RUN_TESTS" = true ]; then
