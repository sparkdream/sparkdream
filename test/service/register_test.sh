...
# Bond is a bare math.Int in the chain's bond denom (resolved at runtime
# from x/identity); the CLI no longer accepts a coin string here.
BOND_COIN="${MIN_BOND_AMT}"
echo "Reading min_bond: $BOND_COIN ($BOND_DENOM)"
...
OP_INFO=$($BINARY query service operator "$OPERATOR1_ADDR" "$TEST_SERVICE_TYPE" --output json)
STATUS=$(echo "$OP_INFO" | jq -r '.operator.status')
STORED_BOND=$(echo "$OP_INFO" | jq -r '.operator.bond_amount')
...
BY_CTRL=$($BINARY query service operators-by-controller "$COMMONS_POLICY" --output json)
...
# Use DREAM_DENOM which is not BOND_DENOM; this MUST be rejected even
# before the bank send because the proto-level check matches BondDenom.
TX_RES=$($BINARY tx service register-operator \
    "$TEST_SERVICE_TYPE" "$COMMONS_POLICY" "${MIN_BOND_AMT}${DREAM_DENOM}" "$META_OP2_BAD_DENOM" \
    --from operator2 --chain-id $CHAIN_ID --keyring-backend test \
    --fees 5000${BOND_DENOM} -y --output json)
submit_and_wait "$TX_RES"
if ! check_tx_failure "$TX_RESULT"; then
...
TX_RES=$($BINARY tx service register-operator \
    "$TEST_SERVICE_TYPE" "$COMMONS_POLICY" "${LOW_BOND_AMT}${BOND_DENOM}" "$META_OP2_LOW_BOND" \
    --from operator2 --chain-id $CHAIN_ID --keyring-backend test \
    --fees 5000${BOND_DENOM} -y --output json)
submit_and_wait "$TX_RES"
if ! check_tx_failure "$TX_RESULT"; then
...
BY_TYPE=$($BINARY query service operators-by-service-type "$TEST_SERVICE_TYPE" --output json)
