...
# ============================================================================
# X/VOTE MODULE E2E TEST SUITE
# ============================================================================
# This script runs all vote module e2e tests in sequence.
#
# Usage:
#   ./run_all_tests.sh                      # Run all tests
#   ./run_all_tests.sh --no-setup           # Skip account setup
#   ./run_all_tests.sh --no-registration    # Skip voter registration tests
#   ./run_all_tests.sh --no-proposal        # Skip proposal tests
#   ./run_all_tests.sh --no-vote            # Skip vote tests
#   ./run_all_tests.sh --no-sealed          # Skip sealed vote tests
...
for arg in "$@"; do
    case $arg in
        ...
        --help|-h)
            ...
            echo "  --no-registration   Skip voter registration tests"
            echo "  --no-proposal       Skip proposal tests"
            echo "  --no-vote           Skip vote tests"
            echo "  --no-sealed         Skip sealed vote tests"
            echo "  --no-query          Skip query tests"
            echo "  --no-tle            Skip TLE tests"
...
echo "============================================================================"
echo "                     X/VOTE MODULE E2E TEST SUITE"
echo "============================================================================"
echo ""
...
if [ "$RUN_SETUP" = true ]; then
    ...
    if [ "$SAVE_SETUP" = true ]; then
        ...
        echo "  bash test/vote/run_all_tests.sh --restore-setup"
        echo ""
...
if [ "$RUN_REGISTRATION" = true ]; then
    run_test "Voter Registration Tests" "voter_registration_test.sh"
else
    echo "Skipping voter registration tests (--no-registration)"
    echo ""
fi

# Proposal tests
if [ "$RUN_PROPOSAL" = true ]; then
    run_test "Proposal Tests" "proposal_test.sh"
else
    echo "Skipping proposal tests (--no-proposal)"
    echo ""
fi

# Vote tests (depends on proposal tests for proposal IDs)
if [ "$RUN_VOTE" = true ]; then
    run_test "Vote Tests" "vote_test.sh"
else
    echo "Skipping vote tests (--no-vote)"
    echo ""
fi

# Sealed vote tests
if [ "$RUN_SEALED" = true ]; then
    run_test "Sealed Vote Tests" "sealed_vote_test.sh"
else
    echo "Skipping sealed vote tests (--no-sealed)"
    echo ""
fi
...
if [ "$RUN_TLE" = true ]; then
    run_test "TLE Tests" "tle_test.sh"
else
