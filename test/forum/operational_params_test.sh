...
if [ "$QUERY_PARAMS_RESULT" == "PASS" ]; then
    ...
    '{
      ...
      proposers: [$alice],
      title: "Update Forum Operational Params",
      summary: "Adjust TTL and spam tax via Council",
      messages: [{
    ...
    SUBMIT_RES=$($BINARY tx group submit-proposal "$PROPOSAL_DIR/update_forum_op_params.json" \
        --from alice -y --chain-id $CHAIN_ID --keyring-backend test \
...
if [ "$UPDATE_PARAMS_RESULT" == "PASS" ]; then
    ...
    '{
      group_policy_address: $policy,
      proposers: [$alice],
      title: "Reset Forum Operational Params",
    ...
    SUBMIT_RES=$($BINARY tx group submit-proposal "$PROPOSAL_DIR/reset_forum_op_params.json" \
        --from alice -y --chain-id $CHAIN_ID --keyring-backend test \
        --fees 5000000uspark --output json)
    TX_HASH=$(echo $SUBMIT_RES | jq -r '.txhash')
